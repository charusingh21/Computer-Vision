...
class SFMSolver(object):
    ...
    def triangulate(self, p1, p2, R, trans, mask):
        ...
        print('camera matrix for first camera',proj_pt1)
        print('camera matrix for second camera',proj_pt2)
    ...
    def run(self):
        ...
        self.visualize_matches(
            self.imgs[1], self.imgs[2], kp1, kp2, good_matches,
            save_path=join(self.output_dir, 'sift_match_1_2.png'))

        # step 3: compute essential matrix
        E, mask = self.compute_essential(p1, p2)

        self.visualize_matches(
            self.imgs[1], self.imgs[2], kp1, kp2, good_matches, mask=mask,
            save_path=join(self.output_dir, 'inlier_match_1_2.png'))

        self.visualize_epipolar_lines(
            self.imgs[1], self.imgs[2], p1, p2, E,
            save_path=join(self.output_dir, 'epipolar_lines_1_2.png'))
        ...
        print('length of 3d points',len(point_3d))
        


        self.write_simple_obj(point_3d, None, filepath=join(
            self.output_dir, 'output_1_2.obj'))
...
def main():
    ...
    img_pattern = r'./cs677_hw3_data/rdimage.???.ppm'
    intrinsic = intrinsic_reader(r'./cs677_hw3_data/intrinsics.txt')
    output_dir = './output'
